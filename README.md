...
| 相補性条件 | $x_{ij}=1 \Rightarrow u_i + v_j = c_{ij}$ | 零要素上のみマッチングを許可 |

終了時点でKKT条件が全て成立するため、最適性が保証されます。

**δ操作の意味：**  
ラベル付き行・列のポテンシャルをδだけ更新することで、既存の零要素（相補性条件を満たす候補）を壊さずに新しい零要素を生成します。このδ更新のたびに双対目的関数値は厳密に増加し、最終的に主双対ともに最適な解へ収束します。
...
- [ハンガリアン法をPythonでスクラッチ実装する：LP双対性・KKT条件から最適性を理解する](https://qiita.com/Haru8-8/items/ebd446a17353d7d57dc3)
- [MIPによる割当問題の拡張：1対多割当のPython実装](https://qiita.com/Haru8-8/items/f3633c1e5902750df20c)
